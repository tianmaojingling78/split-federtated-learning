...
def train_edge(fx_client, labels, idx, len_batch):
    global net_glob_edge, criterion, device, batch_acc_train, batch_loss_train, fed_check
    ...
    net_glob_edge.train()
    optimizer_edge = torch.optim.Adam(net_glob_edge.parameters(), lr=lr)
    ...
    # calculate accuracy
    acc = calculate_accuracy(fx_edge, labels)
    ...
    # edge-side model net_glob_edge is global so it is updated automatically in each pass to this function
    ...
    if count1 == len_batch:
        ...
        if len(idx_collect) == num_clients:
            fed_check = True  # for evaluate_edge function  - to check fed check has hitted
            # all users served for one round ------------------------- output print and update is done in evaluate_edge()
            # for nicer display 
...
class Client(object):
    ...
    def train(self, net):
        ...
        for batch_idx, (images, labels) in enumerate(self.ldr_train):
            ...
            # Sending activations to edge and receiving gradients from edge
            dfx = train_edge(client_fx, labels, self.idx, len_batch)
    ...
    def evaluate(self, net, ell):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_test):
                ...
                evaluate_edge(fx, labels, self.idx, len_batch, ell)
